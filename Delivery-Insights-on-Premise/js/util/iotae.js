...
class iotae {
  ...
  /**
   * Process material deliveries from S4 on premise
   */
  static processOpS4(req, userDestination) {
    var ts = 20200714140101;
    var start = new Date();
    return new Promise(function(resolve, reject) {
      s4op.getMatDeliveries(ts,userDestination)
      .then(materialDeliveries => {
        ...
        .catch((error) => {
          console.log(error);
          reject(new Error("Error processing deliveries in Leonardo IoT: " + error.message));
        });
      })
      .catch((error) => {
          console.log(error);
        reject(new Error("Error getting deliveries from S/4 HANA: " + error.message));
       });
    });
  }
  ...
  static getThingByAlternateId(token, pmd) {
    return new Promise(function(resolve, reject) {
      ...
      var ioTIntegrationConfig = JSON.parse(process.env.IoTIntegration);
      var url = iotae.iotAEConfig().endpoints["appiot-mds"] + "/ThingsByAlternateId('" + pmd.thingAlternateId + "')";
  ...
  static getAllThingsByAlternateId(token) {
    return new Promise(function(resolve, reject) {
      ...
      var ioTIntegrationConfig = JSON.parse(process.env.IoTIntegration);
      var url = iotae.iotAEConfig().endpoints["appiot-mds"] + "/Things?$filter=startswith(_alternateId,'MaterialT')";
